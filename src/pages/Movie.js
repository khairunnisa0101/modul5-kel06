...
export default function Movie() {
    ...
    // Fungsi untuk navigasi ke halaman detail movie
    const handleMovieClick = (id) => {
        navigate(`/movie/${id}`); // Navigasi ke halaman detail movie berdasarkan id
    };
                ...
                {poster.map((item, index) => (
                    <Fragment key={item.id}>
                        <CardBig
                            ...
                            size={STAR_SIZE}
                            color={STAR_COLOR}
                            onClick={() => handleMovieClick(item.id)} // Navigasi ketika diklik
                        />
                        {data.length === index + 1 ? (
